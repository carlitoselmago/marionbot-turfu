...
def textPreprocess(input_text):
  ...
  return removeTriplicated((input_text.lower()))
...
def tokenizeAndFilter(inputs, outputs):
    ...
    with open('saved/tokenizer.pickle', 'wb') as handle:
...
def main():
    ...
    import glob
    linesALL=[]
    files = glob.glob( '../data/ready/*.txt' )
    for f in files:
    ...
    if os.path.exists('saved/saved_weights.h5'):
        model.load_weights('saved/saved_weights.h5')
        print("Trained model found, resuming training","::::::::::")
